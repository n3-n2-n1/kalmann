...
class KalmanFilter {
    ...
    state;
    constructor() {
        this.logger = new logger_1.Logger('KalmanFilter');
     ...
     */
    async predict(klines, lookAhead = 5) {
        try {
            if (klines.length < 10) {
     ...
     */
    extrapolateFuture(predictions, lookAhead) {
     ...
     */
    adaptFilterParameters(volatility, volumeTrend) {
     ...
     */
    calculateConfidence(predictions, actualPrices) {
        if (predictions.length !== actualPrices.length)
            return 0;
