...
export class TechnicalAnalysis {
  ...
  detectCandlePatterns(klines: Kline[]): {
    patterns: Array<{ type: string; confidence: number; suggests: string; reason: string }>;
    lastCandles: Array<{ color: string; bodySize: number; volume: number; timestamp: number }>;
  } {
    if (klines.length < 10) {
      return { patterns: [], lastCandles: [] };
    }

    const patterns: Array<{ type: string; confidence: number; suggests: string; reason: string }> = [];
  ...
  analyzeOrderBook(orderBook: {
    ...
    bidWalls: Array<{ price: number; quantity: number }>;
    askWalls: Array<{ price: number; quantity: number }>;
  ...
  compareTrends(klines1m: Kline[], klines5m: Kline[]): {
    ...
    const divergence = 
      (macroTrend === 'bullish' && microTrend === 'bearish') ||
